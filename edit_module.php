...
if (!isset($_GET['id'])) {
    die("Error: Module ID is not specified.");
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $update_query = "UPDATE modules SET title = ? WHERE id = ?";
    $stmt = $conn->prepare($update_query);
    $stmt->bind_param('si', $title, $module_id);

    if ($stmt->execute()) {
        // Clear existing lessons for the course
        $delete_lessons_query = "DELETE FROM modules_lessons WHERE module_id = ?";
        ...
        try {
            ...
            foreach ($selected_lessons as $lesson_id) {
                ...
                $insert_lesson_query = "INSERT INTO modules_lessons (module_id, lesson_id) VALUES (?, ?)";
                $stmt = $conn->prepare($insert_lesson_query);
                $stmt->bind_param('ii', $module_id, $lesson_id_int);
                $stmt->execute();
            }
        } catch (Exception $e) {
            echo "Error inserting lessons: " . $e->getMessage();
        }
...
$selected_lessons_query = "SELECT lesson_id FROM modules_lessons WHERE module_id = ?";
$selected_lessons_stmt = $conn->prepare($selected_lessons_query);
$selected_lessons_stmt->bind_param('i', $module_id);
...
$query = "
    SELECT * FROM lessons
    WHERE id NOT IN (
        SELECT lesson_id FROM modules_lessons WHERE module_id = ?
    )
";
$stmt = $conn->prepare($query);
$stmt->bind_param('i', $module_id);
...
<head>
    ...
    <title>Academy | Edit Module</title>
        ...
        <div class="row">
            ...
            <main role="main" class="col-md-9 ml-sm-auto col-lg-10 p-0">
                ...
                <div class="container my-4">
                    <h1 class="header-title">Edit Module</h1>
                    <hr>
                    <div class="card">
                        <div class="row align-items-center">
                            <form action="./edit_module.php?id=<?= htmlspecialchars($module_id) ?>" method="post" class="col-12">
                                                ...
                                                <?php if (!empty($selected_lessons)): ?>
                                                    <?php foreach ($selected_lessons as $lesson_id): ?>
                                                        <div class="lesson-item d-flex justify-content-between align-items-center" data-lesson-id="<?= htmlspecialchars($lesson_id) ?>" draggable="true">
                                                            <span><?= htmlspecialchars(getLessonNameById($lesson_id, $conn)) ?></span>
...
function getLessonNameById($lesson_id, $conn)
{
...
// Function to get quiz name by ID
function getQuizNameById($quiz_id, $conn)
{
    ...
    $stmt->execute();
    $result = $stmt->get_result();
    return $result->fetch_assoc()['title'] ?? 'Unknown quiz';
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($stmt->execute()) {
        ...
        foreach ($selected_lessons as $lesson_id) {
            ...
            $insert_lesson_query = "INSERT INTO modules_lessons (module_id, lesson_id) VALUES (?, ?)";
            $stmt = $conn->prepare($insert_lesson_query);
            $stmt->bind_param('ii', $module_id, $lesson_id_int); // Use the variable
            $stmt->execute();
        }
